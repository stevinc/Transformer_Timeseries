...
from models.transformer.utils import generate_local_map_mask
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        # Scaled Dot Product
        self._scores = torch.bmm(queries, keys.transpose(1, 2)) / np.sqrt(cols)

        # Compute local map mask
        if self._attention_size != 0:
            attention_mask = generate_local_map_mask(rows, cols, self._attention_size, mask_future=False, device=self._scores.device)
            self._scores = self._scores.masked_fill(attention_mask, float('-inf'))

        # Compute future mask
        if mask == "future":
            future_mask = torch.triu(torch.ones((rows, cols)), diagonal=1).bool()
...
class MultiHeadAttentionChunk(MultiHeadAttention):
    ...
    def __init__(self,
        ...
        if self._attention_size is not None:
            self._attention_mask = nn.Parameter(generate_local_map_mask(self._chunk_size, self._chunk_size, self._attention_size),
                                                requires_grad=False)
...
class MultiHeadAttentionWindow(MultiHeadAttention):
    ...
    def __init__(self,
        ...
        if self._attention_size is not None:
            self._attention_mask = nn.Parameter(generate_local_map_mask(self._window_size, self._window_size, self._attention_size),
                                                requires_grad=False)
...
def main():
    mhd = MultiHeadAttention(128, 5, 5, 4, attention_size=0)
    k = torch.rand((64, 5, 128))
    q = torch.rand((64, 5, 128))
    v = torch.rand((64, 5, 128))

    attention = mhd.forward(q,k,v, mask='future')
    return attention
